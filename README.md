...
3. The app shows the plugin inventory; the sessions screen lists the host's conversations (create and delete phone-created ones), and the conversation screen loads the history, sends messages, and streams the assistant reply with tool-call rows; the settings tab renders every namespace as a JSON view (editing is not implemented yet).
